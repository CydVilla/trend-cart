...
const EvaluationSchema = z.object({
  ...
  posterNamedProduct: z.boolean().nullable(),
...
4. recommendedSearchQuery: when the post centers on a SPECIFIC identifiable product (game title, device, book), give a short Amazon search query for it (2-8 words). CRAFT the query for good landings: canonical product or franchise name, plus a disambiguating word when the name is generic ("silksong" → "hollow knight silksong switch"; a novel → title + author). RETRO/LEGACY GAMES — hard rule from the operator, and the PLATFORM FLOOR is explicit: the bot recommends video games ONLY for the PS4 / Xbox One / Nintendo Switch generation and newer (PS4/PS5, Xbox One/Series, Switch/Switch 2, current PC/Steam Deck). EVERYTHING older is legacy — PS3, Xbox 360, Wii U, Wii, GameCube, N64, SNES, NES, PS2/PS1, original Xbox, Dreamcast/Genesis and earlier, and EVERY pre-Switch handheld (Game Boy line, DS, 3DS, PSP, Vita). A legacy title must NEVER be queried or recommended as the original copy — nobody buying on Amazon wants a decades-old cartridge or disc at used/collector prices ("The Game Boy version is worth grabbing" is exactly the reply the operator rejected). Query the MODERN edition instead: the current remaster, remake, re-release, or collection on the platforms above ("final fantasy x" nostalgia post → "final fantasy x x-2 hd remaster", NOT "final fantasy x ps2"), and say so in suggestedReplyAngle so the reply recommends the modern edition explicitly. Only when no modern edition exists may you retarget to in-franchise merch/soundtrack per the rules below — and if nothing modern clears the bar, shouldReply=false; a query naming legacy hardware or targeting an original legacy copy gets linkConfidence 20 at most. The floor applies to HARDWARE and accessories too: no legacy consoles, controllers, or add-ons — only current-generation gear is recommendable. If the exact item is not something a buyer can order on Amazon — a digital-only game, streaming show, or service; a title with no listing yet (unannounced or "still in development", no release date and no pre-order page); or an OUT-OF-PRINT, discontinued, collector-only item — RETARGET the query to what IS buyable in the same franchise: a current re-release or new edition, the successor console, the physical edition, official merch, soundtrack vinyl, artbook, or the book it's based on — and say so in suggestedReplyAngle. An upcoming item with a genuine Amazon PRE-ORDER page IS buyable — keep it, and note "pre-order" in suggestedReplyAngle so the reply doesn't imply it already ships.
...
4b. posterNamedProduct: did the AUTHOR identify the product, or did YOU? Set true when the post itself names the item plainly enough that a reader already knows what to buy — the title in the text, a caption like "Game: X / Platform: Y", or a hashtag that spells it out (#ResidentEvilRequiem). Set false when YOU did the identifying work: the author showed or discussed something without naming it, and you recognised it from an image, its alt text, franchise hashtags, or description (figure photography where the alt text names the manufacturer and line; an unnamed gadget on a desk; "this new controller" with no brand given). Judge the PRODUCT identification only — an author naming the franchise but not the specific item ("some Zelda merch") is still false, because the reader still doesn't know what to buy. Null only when there is no product at all. This decides whether a reply must wait for a discount: when the author already named it, the recommendation adds nothing unless the item is on sale; when you identified it, the reply is worth making at any price.
