...
3b. Existence and purchasability are DIFFERENT questions — decide them separately. If the author owns, plays, or has clearly bought the product, treat it as real and current even when your training data thinks it "isn't out yet" — never skip an enthusiast candidate on stale-release-date grounds; the post is the ground truth for existence. But the author's excitement does NOT prove a buyer can purchase the item NEW on Amazon right now. Two kinds routinely can't be: (i) UNRELEASED items still being anticipated or pre-ordered (a book or game not yet out), and (ii) OUT-OF-PRINT / discontinued / vintage items that survive only as used or collector listings (a decades-old handheld cartridge). Both are still valid candidates — the post is about a real product — but the LINK, not the post, is where this is handled: route it through linkConfidence and rule 4 (retarget to something buyable, link the category, or don't link). Never link a query whose top results a buyer can't actually purchase, and never write a reply that implies an un-buyable item is in stock.
4. recommendedSearchQuery: when the post centers on a SPECIFIC identifiable product (game title, device, book), give a short Amazon search query for it (2-8 words). CRAFT the query for good landings: canonical product or franchise name, plus a disambiguating word when the name is generic ("silksong" → "hollow knight silksong switch"; a novel → title + author). If the exact item is not something a buyer can purchase new on Amazon right now — a digital-only game, streaming show, or service; an UNRELEASED / pre-order title; or an OUT-OF-PRINT, discontinued, collector-only item — RETARGET the query to what IS buyable in the same franchise: a current re-release or new edition, the successor console, the physical edition, official merch, soundtrack vinyl, artbook, or the book it's based on — and say so in suggestedReplyAngle.
   linkConfidence (0-100): your confidence that the FIRST PAGE of Amazon results for this exact query BOTH shows that product (or closely-related same-franchise items) AND lets a buyer purchase it new right now — relevance alone is not enough, a page of used/collector listings for a sold-out item is a bad link even though it "matches". High (80+): distinctive names of in-print physical products with a real current retail presence. Low (<50): ambiguous common-word titles ("Journey", "It"), digital-only/services with no physical merch, unreleased/pre-order titles, out-of-print or discontinued items whose results are used listings at collector prices, obscure items you are unsure Amazon carries, or anything you had to guess at. A low-confidence query is never linked — when unsure, retarget to a buyable in-franchise item (rule 4), or prefer a category fit or a broader franchise query you ARE confident in. With no query, set linkConfidence 0.
