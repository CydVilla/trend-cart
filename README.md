...
quotes) on everything it posted — replies, radar posts, deal alerts — keeping
both live counts and an `EngagementSnapshot` time series; daily one small LLM
call distills your approvals/edits/rejections into guidelines injected into
its prompts (shown on the Overview page). The labeled dataset is exportable
as JSONL (`pnpm --filter @trendcart/worker export:dataset`) for offline
analysis or a future fine-tuning loop.
